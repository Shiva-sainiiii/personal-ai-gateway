  ...
  if (claimed) {
    return {
      ...
      async release(result) {
        // Store the result briefly so waiters can pick it up.
        await ref.set({ claimedAt: FieldValue.serverTimestamp(), result }, { merge: true });

        // Bug fix: this doc used to never get deleted — the comment said
        // "let it expire naturally" but nothing in this file (or anywhere
        // else) actually deletes Firestore documents on a timer; that's not
        // something Firestore does by itself without a TTL policy configured
        // in the console (see README's cleanup section) or an explicit
        // delete call. Every coalesced request left a permanent orphaned
        // "inFlight" doc. Schedule a best-effort delete after the longest
        // any waiter could still be polling for this result (MAX_WAIT_MS +
        // one poll interval of slack) — by then every waiter has either
        // picked up the result or given up and moved on itself, so the doc
        // has served its purpose. This only fires while THIS invocation is
        // still alive; a Firestore TTL policy is still the reliable backstop
        // for docs whose invocation gets killed/cold-started away first.
        setTimeout(() => {
          ref.delete().catch(() => {});
        }, MAX_WAIT_MS + POLL_INTERVAL_MS).unref?.();
      },
    };
  }
