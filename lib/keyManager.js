...
export async function markKeyFailure(keyId, errorMessage, isRateLimit) {
  const update = {
    lastUsedAt: FieldValue.serverTimestamp(),
 ...
 */
export async function logRequest({ type, provider, keyId, model, ok, status, latencyMs, errorMessage }) {
  await db()
