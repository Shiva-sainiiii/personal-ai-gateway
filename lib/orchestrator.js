    ...
    for (const entry of rankedModels) {
      ...
      for (const keyDoc of keys) {
        ...
        try {
          ...
          if (result.ok) {
            // Record against the DAILY_FREE_LIMITS account-wide quota tracker.
            // Only meaningful for providers with an actual account-wide
            // bucket (openrouter, cloudflare) — Groq and Google AI Studio are
            // "per-model" scope (see usageLimits.js), so there's no single
            // account-wide number to tally against; their per-model ceilings
            // (Groq: PER_MODEL_LIMITS, checked live in rateLimitSaver.js) are
            // tracked separately instead. Cerebras is account-wide but its
            // free-TRIAL credit balance isn't something this token-count
            // heuristic can represent, so it's also skipped here for now.
            const hasAccountWideCap = entry.provider === "openrouter" || entry.provider === "cloudflare";
            ...
            await Promise.all([
              ...
              hasAccountWideCap ? recordUsage(entry.provider, keyDoc.id, 1) : Promise.resolve(),
            ]);
...
export async function routeAudioRequest({ audioBuffer, contentType }) {
  ...
  for (const providerName of AUDIO_PROVIDER_ORDER) {
    ...
    const model = providerName === "googleAiStudio" ? "gemini-3.6-flash" : provider.freeModels.audio[0];
...
/**
 * Manual/forced routing — bypasses pool selection, the self-healing scorer,
 * caching, and coalescing entirely. Used by the admin Test page's manual
 * provider/model picker: the person explicitly chose a provider and model,
 * so the point is to hit exactly that pair, not let auto-routing override
 * it. Still falls back across that ONE provider's own keys/accounts (a key
 * being on cooldown isn't something the person is trying to test around),
 * but never crosses to a different provider — that would defeat the purpose
 * of a manual test.
 *
 * Logged with type "manual-test-<text|image|audio>" (not "text"/"image"/
 * "audio") so these calls are visually distinguishable from real traffic in
 * Recent Requests and don't get counted as organic usage in the dashboard's
 * last-24h success/fail totals or model-scorer outcomes.
 */
export async function routeForcedRequest({ type, provider: providerName, model, messages, prompt, imageUrl, imageBase64, imageMimeType, audioBuffer, contentType }) {
  const provider = PROVIDERS[providerName];
  if (!provider) {
    return { ok: false, error: `Unknown provider "${providerName}".`, attempts: [] };
  }
  ...
  if (keys.length === 0) {
    return { ok: false, error: `No active keys stored for provider "${providerName}".`, attempts: [] };
  }

  const logType = `manual-test-${type}`;
  ...
  for (const keyDoc of keys) {
    ...
    try {
      ...
      if (type === "text") {
        ...
        const audioBase64 = audioBuffer.toString("base64");
        if (providerName === "googleAiStudio") {
          result = await provider.transcribeAudio({ apiKey, model, audioBase64, mimeType: contentType });
        } else if (providerName === "cloudflare") {
          // Cloudflare Whisper wants raw bytes on the wire, not a JSON/base64
          // envelope — same special-cased path as routeAudioRequest above.
          const url = provider.baseUrlFor(keyDoc.accountId, model);
          const controller = new AbortController();
          const timeoutId = setTimeout(() => controller.abort(), 15000);
          let res;
          ...
          const json = await res.json().catch(() => null);
          result = res.ok
            ? { ok: true, text: json?.result?.text ?? "" }
            : { ok: false, status: res.status, error: json };
        } else {
          return { ok: false, error: `Provider "${providerName}" doesn't support audio transcription.`, attempts: [] };
        }
      } else {
        return { ok: false, error: `Unknown test type "${type}".`, attempts: [] };
      }
      ...
      if (result.ok) {
        if (!keyDoc.noKey) await markKeySuccess(keyDoc.id);
        await logRequest({ type: logType, provider: providerName, keyId: keyDoc.id, model, ok: true, latencyMs, usage: result.usage });
        return {
          ok: true,
          text: result.text,
      ...
      attempts.push({ provider: providerName, keyId: keyDoc.id, status: result.status, error: result.error });
    } catch (err) {
      if (!keyDoc.noKey) await markKeyFailure(keyDoc.id, err.message, false);
      attempts.push({ provider: providerName, keyId: keyDoc.id, error: err.message });
    }
  ...
  return { ok: false, error: `All ${providerName} keys failed for model "${model}".`, attempts };
}
