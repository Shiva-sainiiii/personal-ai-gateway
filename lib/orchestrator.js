...
//
// googleAiStudio's gemini-2.5-flash-image ("Nano Banana") is kept as a
// secondary attempt in case a given Google AI Studio key IS entitled to it
// (this varies by account/region and can change), but it is NOT relied on —
// Google's free tier normally only grants VISION (image input, not output)
// access, which is exactly the bug this fixes.
//
// Cloudflare Workers AI (Stable Diffusion XL) is the final fallback.
const IMAGE_PROVIDER_ORDER = ["pollinations", "googleAiStudio", "cloudflare"];
  ...
  for (const providerName of IMAGE_PROVIDER_ORDER) {
    ...
    for (const keyDoc of keys) {
      ...
      try {
        ...
        const rateLimited = isRateLimitError(result.status);
        const permanent = isPermanentError(result.status);
        if (!keyDoc.noKey) await markKeyFailure(keyDoc.id, JSON.stringify(result.error), rateLimited, permanent);
        await logRequest({
          type: "image",
