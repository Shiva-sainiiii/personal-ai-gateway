...
export const PROVIDERS = {
  openrouter: {
    ...
    // quota_scope: account-wide — ALL :free models below share ONE bucket
    // per account/key. 20 RPM / 50 RPD with no lifetime credits purchased;
    // 20 RPM / 1,000 RPD once $10+ lifetime credits have ever been added.
    // List re-verified against OpenRouter's own model catalog as of Aug
    // 2026 — a few entries from the previous list (inclusionai/ling-3.0-flash,
    // nvidia/nemotron-3-nano-30b-a3b, nvidia/nemotron-nano-12b-v2-vl,
    // nvidia/nemotron-nano-9b-v2) could not be re-confirmed as currently
    // live :free ids and were dropped rather than guessed at; OpenRouter
    // rotates this list without notice, so re-check openrouter.ai/models
    // periodically regardless of what's below.
    freeModels: [
      "z-ai/glm-5.2:free",
  ...
  googleAiStudio: {
    ...
    // quota_scope: per-model — each model below has its OWN RPM/TPM/RPD
    // bucket on the same project/key, NOT one shared account pool. Google
    // removed the public per-model limit tables from its rate-limits docs
    // in Aug 2026, so exact numbers now only show up per-project in the AI
    // Studio console — there is no reliable published number to hardcode
    // here anymore. What IS confirmed directly from Google's docs: it is
    // RPM *and* TPM *and* a daily RPD cap for every free-tier model (RPD
    // resets at midnight Pacific) — this is NOT an "RPM-only, no daily cap"
    // tier for any model, so don't assume unlimited daily volume here.
    // This model list itself is UNVERIFIED against Aug 2026 docs (Google's
    // own free-tier model list also rotates without notice) — confirm
    // against ai.google.dev/gemini-api/docs/rate-limits and the AI Studio
    // console before relying on any specific model id here staying free.
    freeModels: [
      "gemini-3.7-flash",
      "gemini-3.6-flash",
  ...
  groq: {
    ...
    // quota_scope: per-model — every model below has its OWN separate
    // RPM/RPD/TPM/TPD bucket, even on the same account/key. Exhausting
    // openai/gpt-oss-120b's quota does NOT touch llama-3.1-8b-instant's —
    // exact confirmed numbers live in usageLimits.js's PER_MODEL_LIMITS.groq.
  ...
  cerebras: {
    ...
    // IMPORTANT (verified Aug 2026): Cerebras no longer has a standing free
    // tier. Signup grants a $5 free-TRIAL credit that EXPIRES 30 days after
    // issuance, and — as of Aug 2026 — a verified payment method is now
    // MANDATORY just to activate API/Playground access at all; without one,
    // the account sits "inactive" and every call here will fail regardless
    // of key validity. quota_scope is account-wide (5 RPM / 30,000 TPM /
    // 1,000,000 tokens-per-hour / 1,000,000 TPD, shared across whichever
    // free model you call) — there is no per-model quota to fall back to on
    // this provider once the shared trial credit or its 30-day window runs
    // out. Re-verify at cerebras.ai before relying on any Cerebras account
    // long-term; a key that suddenly 401s/402s here is more likely "trial
    // expired" than "key revoked".
  ...
  pollinations: {
    ...
    // As of Pollinations' new model catalog (enter.pollinations.ai/models),
    // "flux"/"turbo"/"flux-realism" no longer exist — every image model now
    // carries an explicit Quest (free) or Paid tag, and sending an unlisted/
    // stale model id is what was causing the 401/402s. "dreamshaper" is
    // currently the only Quest-tagged plain image model (nova-canvas is also
    // Quest but gates output at a "1024 tier" resolution setting, which adds
    // complexity this gateway doesn't need). Re-check enter.pollinations.ai/models
    // periodically since Pollinations has already renamed this list once.
    freeModels: { image: ["dreamshaper"] },
    ...
    async generateImage({ apiKey, prompt, model = "dreamshaper" }) {
  ...
  cloudflare: {
    ...
    // quota_scope: account-wide — 10,000 Neurons/day shared across every
    // model type (text/image/audio) on the account, resets 00:00 UTC.
    // Text list cross-checked against Cloudflare's Workers AI model catalog
    // as of Aug 2026: @cf/meta/llama-3.3-70b-instruct was NOT independently
    // re-confirmed as a currently-live model id and has been kept only as a
    // fallback alongside its newer fp8-fast variant and llama-4-scout
    // (vision-capable) — verify against developers.cloudflare.com/workers-ai/models
    // before assuming either the old or new id is the one still live.
    freeModels: {
      text: [
        "@cf/meta/llama-3.3-70b-instruct-fp8-fast",
        "@cf/meta/llama-4-scout-17b-16e-instruct", // vision-capable (image-input)
        "@cf/openai/gpt-oss-120b",
        "@cf/google/gemma-3-12b-it",
        "@cf/meta/llama-3.3-70b-instruct", // unverified — kept as fallback, may be renamed/retired
